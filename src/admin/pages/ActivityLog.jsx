...
function actionIcon(cls) {
  const map = { login: "→", logout: "←", create: "+", update: "✎", delete: "✕", default: "·" };
  return map[cls] || "·";
}

const ACTION_TYPES = [
  "admin.login", "admin.logout", "admin.create", "admin.update", "admin.delete",
  "unit.create", "unit.update", "unit.delete",
  "queue.update", "queue.delete",
];
...
export function ActivityLog() {
  ...
  const load = useCallback(async (params) => {
    setLoading(true);
    try {
      const res = await api.activity({ ...params, page: params.page ?? 1 });
      setRows(res.data);
  ...
  return (
    <>
      <div style={{ marginBottom: 20 }}>
        ...
        <p className="sa-text-muted sa-text-sm">Full audit trail of all admin actions.</p>
      </div>
      ...
      <div className="sa-card">
        ...
        <div className="sa-filters">
          <div className="sa-search" style={{ minWidth: 220 }}>
            ...
            <input placeholder="Search description, admin…" value={filters.search} onChange={setFilter("search")} />
          </div>
          <select className="sa-select" value={filters.action} onChange={setFilter("action")}>
            <option value="">All Actions</option>
            {ACTION_TYPES.map((a) => <option key={a} value={a}>{a}</option>)}
          </select>
          <select className="sa-select" value={filters.admin_id} onChange={setFilter("admin_id")}>
            <option value="">All Admins</option>
            {admins.map((a) => <option key={a.admin_id} value={a.admin_id}>{a.admin_name}</option>)}
          </select>
          <select className="sa-select" value={filters.entity} onChange={setFilter("entity")}>
          ...
          <button className="sa-btn sa-btn-outline sa-btn-sm" onClick={() => setFilters({ search: "", action: "", admin_id: "", entity: "", from: "", to: "" })}>
            Clear
          </button>
          <span className="sa-text-muted sa-text-sm" style={{ marginLeft: "auto", whiteSpace: "nowrap" }}>
            {pag.total} event{pag.total !== 1 ? "s" : ""}
          </span>
        </div>
        ...
        {loading ? (
          ...
          <div className="sa-empty"><div className="sa-empty-icon">📋</div><div className="sa-empty-text">No activity found.</div></div>
        ) : (
          <div style={{ padding: "8px 20px" }}>
            <ul className="sa-activity-list">
              {rows.map((a) => {
                const cls = actionDotClass(a.action);
                return (
                  <li key={a.id} className="sa-activity-item">
                    <div className={`sa-activity-dot ${cls}`}>{actionIcon(cls)}</div>
                    <div className="sa-activity-info">
                      <div className="sa-activity-desc">
                        <strong>{a.admin_name || "System"}</strong>
                        {" — "}
                        {a.description || a.action}
                      </div>
                      <div className="sa-activity-meta">
                        <span className="sa-badge viewer" style={{ fontSize: 10, padding: "1px 6px" }}>{a.action}</span>
